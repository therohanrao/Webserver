...
class NginxConfig {
 public:
  ...
  std::map<std::string, std::string> location_map_;
  int get_port();
  std::string get_root();
  std::string get_data_path();
  int get_max_threads();
  std::map<std::string, std::pair<std::string, std::shared_ptr<NginxConfig>>> get_location_mapping(int& error); //0 = no error, 1 = syntax error
};
